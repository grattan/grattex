...
Although \LaTeX{} doesn't require it, you \emph{must} label all chapters, and every label must have a specific prefix followed by a colon, as throughout this document.

To refer to a chapter, use \Chapref{chap:example}, not \Vref{chap:example}, as the latter's hyperlink points to the baseline of the first text of the chapter, rather than the chapter title.
...
To refer to a footnote, use footnote~\ref{fn:example} \vpageref{fn:example}, rather than \Vref{fn:example}; however, the label \emph{must} be inside the footnote.%

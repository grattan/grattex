...
moretexcs={setlength,chapter,subsection,subsubsection,appendix,addbibresource,FloatBarrier,contentspage,units,includegraphics,noteswithsource,notes,source,bottomrule,midrule,toprule},
columns=flexible,
breaklines=true,
...
\begin{table}
  ...
  \noteswithsource{Excludes Temporary Budget Repair Levy (2\%\ for those earning over \$180,000 until 2016-17)}%
  {Grattan analysis ATO 2\%\ sample file (2013). See \url{https://grattan.shinyapps.io/Changing_income_taxes/Forty_three_app.Rmd}}
\end{table}
  \end{lstlisting}
  \subsection{Boxes}
  ...
  \subsubsection{verysmallbox}\index{verysmallbox}\index{boxes!verysmallbox}
  ...
  % \begin{center}
  {{Your labels should be evocative of what is displayed, {not} the number.} }
  {You will end up moving or removing a figure, table, or box and confuse your labels.}
  % \end{center}

  If you refer to a cross-reference that doesn't exist, the pdf will contain a bold \lstinline!??!. 

  The contents of \lstinline!\label! can be anything containing letters, underscores, or hyphens. 
  I \emph{strongly} recommend you use the prefixes in \Vref{tbl:prefixes-by-float-type}. 
  Using these prefixes consistently will make auto-completion easier. 

  \begin{table}[H]
  \caption{Prefixes to use in cross-reference anchors, by float type\label{tbl:prefixes-by-float-type}}
  \begin{tabular}{>{\ttfamily}l>{\ttfamily}l>{\ttfamily}l}
  \toprule
  \textbf{\textrm{Float environ.}} & \textbf{\textrm{Prefix}} & \textbf{\textrm{Example}} \\
  \midrule 
  figure & fig: & \verb=\label{fig:prop-hholds-by-decile}= \\
  table  & tbl: & \verb=\label{tbl:tax-paid-by-bracket}= \\
  box    & box: & \verb=\begin{smallbox}{A short history of motorsport}{box:motorsport}= \\
  \bottomrule
  \end{tabular}
  \end{table}
  ...
  \subsubsection{Citations}\index{citations}\index{footcite}\index{footcites}
  ...
  \subsection{\texttt{microtype}'s spacing and \texttt{ragged2e}} 

...
\end{kframe}
\end{knitrout}
  \item For checks of a report that is intended for (possibly embargoed) distribution, but not necessarily for release: that the template is up-to-date, the 100th footnote and beyond are correctly formatted, the ISBN is inserted and valid, and there are no editorial marks in the document
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{checkGrattanReport}\hlstd{(}\hlkwc{compile} \hlstd{=} \hlnum{TRUE}\hlstd{,} \hlkwc{final} \hlstd{=} \hlnum{TRUE}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{checkGrattanReport}\hlstd{(}\hlkwc{compile} \hlstd{=} \hlnum{TRUE}\hlstd{,} \hlkwc{final} \hlstd{=} \hlnum{TRUE}\hlstd{,} \hlkwc{release} \hlstd{=} \hlnum{TRUE}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
\end{enumerate}

\section{System requirements for release}
\begin{enumerate}
\item R and the package \texttt{grattanReporter}, installable via:
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{devtools}\hlopt{::}\hlkwd{install_github}\hlstd{(}\hlstr{"hughparsonage/grattanReporter"}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\section{Project folder structure required for release}

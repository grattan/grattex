...
\begin{tabularx}{\linewidth}{>{\bfseries}p{0.1\linewidth}lrRX>{\centering\arraybackslash}p{0.1\linewidth}}
  ...
  \begin{enumerate}
    ...
    \item \lstinline!\Chapref! same as \lstinline!\Cref! but mandatory for references to chapters \index{Chapref}\label{enum:Chapref}
...
if (!is.na(myLibrary) && identical(substr(myLibrary, 0, 2), c("\\", "\\"))) {
  myLibrary <- tempdir()  # guaranteed to be writeable, if it is possible to install anything
}

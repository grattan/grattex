...
\item \textbf{\texttt{ignore\_spelling\_in\_file:}} You may use the \lstinline!% ignore_spelling_in_file: ! directive followed by the path/name of a \texttt{.tex} file to ignore spelling of that entire file. This should be used with caution. For example, ignoring all spelling in Appendix~A but checking spelling in Chapters 1 and 2:
\begin{lstlisting}
\documentclass{grattan}

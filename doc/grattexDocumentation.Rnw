...
Additional arguments are provided for additional checks
\begin{enumerate}
...
@
  \item For checks of a report for release: embeds the fonts and copies the file to the \texttt{RELEASE} folder.
\end{enumerate}

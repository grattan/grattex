...
\section{Using \texttt{pandoc}}\label{sec:using-pandoc}
\texttt{pandoc} comes installed with RStudio.
To convert \texttt{orig.docx} to \texttt{new.tex}, run from the directory containing \texttt{orig.docx}.
\begin{lstlisting}[language=bash]
pandoc -s orig.docx -s --wrap=none --top-level-division=chapter -o new.tex
\end{lstlisting}

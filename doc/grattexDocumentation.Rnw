...
\newcommand{\oneraggedpage}{\let\mytextbottom\@textbottom
  ...
  \subsection{Figure numbers exceed 4 digits and overlap captions}
  The parameter \lstinline!\cftfignumwidth! (defined the \texttt{tocbasic} package)\footnote{See the \texttt{scrguien.pdf} documentation for further details.}
  is the amount of space reserved for the figure number in the list of figures. 
  If the figure number reaches 10.10, the width may overlap the captions, so you will need to increase this parameter:

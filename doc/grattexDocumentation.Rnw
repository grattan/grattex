\documentclass[a4paper,numbers=noenddot,11pt]{scrreprt}
...
\RedeclareSectionCommand[tocnumwidth=3.7em, tocindent=4.7em]{subsection}
...
It may have mandatory or optional arguments, which occur immediately after \verb=\begin{environment}=.  
 ...
 \caption{Examples of environments}
  ...
  \part{User guide for report authors}
  \addsec{Getting started}\label{sec:getting-started}
...
\end{document}
  ...
  You can also use \lstinline!\begin{summary}! as required. If you want to change the name of the title, ask us --- it is straight-forward to amend.
  ...
  You can use \lstinline!\recommendation! to title individual recommendations within this environment.
  ...
  \chapter{Figures, floats and boxes}
  \section{Figures}
...
\begin{tabularx}{\linewidth}{>{\bfseries}p{0.1\linewidth}lrRX>{\centering\arraybackslash}p{0.1\linewidth}}
  ...
  The database is a text file, with file extension \lstinline!.bib!, containing entries for each work to be cited:
...
Enter the date if possible; the year if not. If you only have the month and year, you can enter the month as an integer.
  ...
  \subsection{Miscellaneous bibliography entry quirks}
  If your URL contains special characters, \emph{do not escape them}. They will be printable but point to the wrong address. 
...
\subsection{\texttt{todotnotes}}\index{\texttt{todonotes}}
The package \lstinline!todonotes! provides handsome comment features

The following should be put in \verb=tex/preamble=
\begin{lstlisting}
...
\subsection*{How to read this style guide}
Marginal notes \releaseonly \prereleaseonly beside a rule signify that the rule is only enforced at release or pre-release. 
...
In ShareLaTeX, click the hambuger menu button at top left, then \verb=GitHub=, \verb=Push changes to GitHub=, then \verb=Commit=.

\subsubsection{Implementation}

\subsection{Annunciation}
By default, any problems with your report will be annuciated only at grattex slack \verb=grattex.slack.com=. 
...
\caption{List of all commands} \\
    ...
    vpageref & Refernce to a page (including preposition). & Cross-referencing \\

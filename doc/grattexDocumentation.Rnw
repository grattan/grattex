...
\begin{enumerate}
  ...
  \item If your report was started before February 2007---run \lstinline{library(grattanReporter); checkGrattanReport('@@<path/to/your/project>@@', compile = TRUE, pre_release = TRUE, release = TRUE, embed = @@<TRUE|FALSE>@@)} and then deploy the file written in the \texttt{RELEASE} subdirectory.
  \item If your report was started after February 2007---click download in ShareLaTeX.
\end{enumerate} 

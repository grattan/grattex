...
<<update_grattanReporter, echo=FALSE, eval=TRUE>>=
update.packages("grattanReporter", repos = "https://hughparsonage.github.io/drat")
grattex_version <- 
  readLines("../grattan.cls") %>%
  grep(pattern = "\\ProvidesClass{grattan}", fixed = TRUE, value = TRUE) %>%
  sub(x = ., pattern = "^.* (v[1-9]\\.[0-9]+\\.[0-9]+) .*$", "\\1")
@
\subtitle{Manual for \texttt{grattex} \Sexpr{grattex_version} and \texttt{grattanReporter} v\Sexpr{packageVersion("grattanReporter")}}
...
<<expected-competencies, results='asis', echo=FALSE, eval=FALSE>>=
Expected_Competency <-
  fread("expected-competencies.tsv", showProgress = FALSE) %>%
  .[, big_group := factor(big_group, 
                          levels = c("Initial report stage/conversion/general",
                                     "Editing stage",
                                     "Final stage/publication"),
                          ordered = TRUE)] %>%
  # Won't be santized:
  .[, Competency := gsub("(?<!(?:\\\\verb.))\\\\((?!verb)\\w+)\\b", "\\\\verb!\\\\\\1!", Competency, perl = TRUE)] %>%
  setorderv(c("big_group", "PD/Fellow/Editor", "Associate", "LaTeX guru"),
            order = c(1, -1, -1, -1)) %>%
  .[, lapply(.SD, function(x) if (is.integer(x) && uniqueN(x) <= 2L) as.logical(x) else x)] %>%
  # str
  .[, lapply(.SD, if_else, "\\parbox[c]{2cm}{\\centering\\CheckmarkBold}", ""), by = c("big_group", "Competency")]

first_groups <-
  copy(Expected_Competency)[, I := .I][, .(R = range(I)), keyby = "big_group"]

res <- 
  kable(Expected_Competency[, big_group := NULL],
        format = "latex",
        align = c("p{0.5\\linewidth}", "R", "R", "R"),
        escape = FALSE,
        booktabs = TRUE) %>%
  kable_styling() %>%
  group_rows(first_groups[["big_group"]][1],
             first_groups[["R"]][1],
             first_groups[["R"]][2]) %>%
  group_rows(first_groups[["big_group"]][3],
             first_groups[["R"]][3],
             first_groups[["R"]][4]) %>%
  group_rows(first_groups[["big_group"]][5],
             first_groups[["R"]][5],
             first_groups[["R"]][6])

cat(c(as.character(res), "\n"), file = "expected-competencies-tabularx.tex")
@

\newgeometry{left=0.25cm,right=0.25cm}
\addsec{Expected \LaTeX{} competencies}
% kable not perfect, needs to be modified
\input{expected-competencies-tabularx-manual}

\clearpage
\begin{longtable}{@{\texttt{\textbackslash}}>{\ttfamily}l>{\raggedright}p{0.55\textwidth}l}
\caption{List of all commands}\label{tbl:all-cmds} \\
    \toprule
...
\end{lstlisting}
\end{minipage} \\ 
Small box centred in the page & \begin{minipage}[t]{0.45\linewidth}
\begin{lstlisting}
\begin{centredsmallbox}{@@<Box title>@@}{box:@@<xref-key>@@}

\end{centredsmallbox}
\end{lstlisting}
\end{minipage} \\ 
Small unnumbered box & \begin{minipage}[t]{0.45\linewidth}
\begin{lstlisting}
\begin{centredsmallbox}{@@<Box title>@@}{box:@@<xref-key>@@}
...
\begin{enumerate}
  \item By default, the browser will display misspellings using the en-US dictionary. 
  To change this, go to Menu, and next to Spelling change the language.
\end{enumerate}



\section{Installation}\label{sec:installation-of-latex}
Share\LaTeX{} is available through a browser; \ie~no `local' installation is required.
However, if you require a local installation visit 
...
It's also important that it be kept up-to-date. See the following link for up-to-date instructions on updating your distributions.

\url{https://tex.stackexchange.com/questions/55437/how-do-i-update-my-tex-distribution}
  ...
  Experience has shown that it can be difficult to predict the final title or even publication \emph{year}, it's best to refer to the subject of the report rather than a quippy title.
  Use the following repository naming convention: 
  \begin{quote}
  \lstinline!@@<Program>@@-@@<Start year>@@-@@<Report subject>@@!
  \end{quote}

  Archived reports should use the prefix \lstinline!zzz!.
  ...
  \item They will perform the following steps:
    \begin{enumerate}
      ...
      \item Ensure you have been added to the \url{https://github.com/grattan} organizational account.
      ...
      \item Change \textbf{Owner} to \texttt{grattan}.
      ...
      \item Add collaborators as desired. In particular:
      \begin{enumerate}
        \item add the \LaTeX nicians (Carmela, Hugh, Kate, Lucy, and Will); and
        \item copy the \texttt{Anyone with this link can edit the project} link and paste it
              just under \verb=\documentclass{grattan}= as a comment, \textit{i.e.}
              \begin{lstlisting}
      ...
      \end{enumerate}
      \item Go to \url{https://travis-ci.com/profile/grattan/}. 
      Find the repository you just added and flick the switch on. 
      \item Return to Share\LaTeX{}, comment out the \lstinline!\nocite{*}! and sync to GitHub.
    \end{enumerate}
  \end{enumerate}
...
\node (grattex-GitHub) [decision, above of = GitHub, node distance = 6cm, text width = 5cm]{\url{https://github.com/grattan/grattex} Template elements and configuration files};
...
\section{Word document conversion using \texttt{pandoc}}\label{sec:using-pandoc}

  You can use \texttt{pandoc} to convert Word documents to \LaTeX\ documents. It comes installed with RStudio and can be run from \texttt{terminal} (on Mac) or \texttt{cmd} (on Windows). If it hasn't been installed, you can install it at \url{https://pandoc.org/installing.html}. It requires a local version of \TeX. Converting a Word document to \LaTeX\ is a breezy three-part process.

  \subsection{Preparing Word document}
  There are some things \texttt{pandoc} should ignore (\eg~the Word Table of Contents, which will be automatically generated in \LaTeX), and some things it's not very good at (\eg~converting cross-references). To make the post-conversion tidying easier, prepare your Word document for conversion. Make a \textbf{copy} of your report, and:

    \begin{enumerate}
      \item Turn track changes off.
      \item Remove the cover-page, table of contents, list of figures and bibliography.
      \item Turn off numbering in all headers. Do this by selecting a \texttt{Heading 1}, removing the numbering, right-clicking on \texttt{Heading 1} in the \texttt{Style Pane} and selecting "Update Heading 1 to Match Selection". Repeat for Headings 2 and 3.
      \item Turn off all hyperlinks. On Mac: select all, then \texttt{cmd+6}. On Windows: select all, then \texttt{ctrl+shift+F9}. Note that you will have to repeat this step for the footnotes (click on any footnote, select all, \dots).
      \item Set the whole document to be single-column.
      \item Save the Word document using a filename without spaces, \eg~\verb|mapping_converted.docx|.
    \end{enumerate}

  \subsection{Running \texttt{pandoc}}
      Next, convert the \texttt{.docx} to a \texttt{.tex} file using \texttt{pandoc}. Open \texttt{terminal} (Mac) or \texttt{cmd} (Windows) and set the working directory to the location of your \texttt{.docx} document by:
      
         \begin{lstlisting}
      cd @@<path to folder containing document>@@
         \end{lstlisting}
      \eg 
         \begin{lstlisting}
      cd /Users/mackeyw/Documents/convert
         \end{lstlisting}

      Now that you have told \texttt{terminal}/\texttt{cmd} where to look, you can run \texttt{pandoc} using the following command. Replace \texttt{report} with the title of your document.
      \begin{lstlisting}
      pandoc -s @@<name>@@.docx -s --wrap=none --top-level-division=chapter 
      -o @@<name>@@.tex
      \end{lstlisting}
    
      \eg 
      \begin{lstlisting}[language=bash]
      pandoc -s mapping_converted.docx -s --wrap=none --top-level-division=chapter -o mapping_converted.tex
      \end{lstlisting}

    This will create a \texttt{.tex} file in your folder. 

  \subsection{Initial compile of \texttt{.tex} file}

    The final step is to get the \texttt{.tex} file to compile before you bring it into ShareLaTeX. This involves `commenting out' a few things that \texttt{pandoc} can't do well (\eg~tables) and some that we haven't given it the information to do at all (\eg~figures/charts).
    
    \begin{enumerate}
        \item   Open the \texttt{.tex} in your local \TeX~program. 
        \item   Using find-replace, search \verb|\includegraphics| and replace with \verb|%%\includegraphics|. This will comment out all figures in the document.
        \item   Using find, search \verb|\begin{longtable}| and comment out everything between and including \verb|\begin{longtable}| and \verb|\end{longtable}|. Note that boxes will have been converted to \verb|longtable| environments, which also need to be commented out.
        \item   \texttt{pandoc} will sometimes add an unnecessary \verb|\hypertarget{}| around chapter/section names. You can manually remove these, or replace using \texttt{regex}:
          \begin{itemize}
                \item Find:     \verb|\\hypertarget{((\w*.*)[\r\n](\w*.*))}|
                \item Replace:  \verb|$3|
          \end{itemize}
  \end{enumerate}

After making these changes, compile your document. Correct any errors preventing the compile (if there are any). When it compiles---yay---you can move the \texttt{.tex} code into your Share\LaTeX~document, starting with the text after \verb|\begin{document}|, ignoring the preamble (see following subsection).

From here: 
\begin{enumerate}
  \item   Add your graphics (see \vref{sec:Figures})
  \item   Add your references (see \vref{chap:citations-and-bibliography})
  \item   Add your tables (see \vref{sec:tables})
\end{enumerate}
  


  \section{The preamble}\label{sec:the-preamble}
...
\end{document}
  ...
  \subsection{Grattan-specific preamble}\label{subsec:Grattan-specific-preamble}
  ...
  \end{lstlisting}
  You can also have a subtitle\index{subtitle} which you invoke using \lstinline!\subtitle!. The subtitle does not appear in the running headers.

  If you wish to use another type of report (like `Background Paper'), you can simply use \lstinline!\ReportOrWorkingPaper{@@<type>@@}!.\index{\texttt{ReportOrWorkingPaper}}
  ...
  \subsection{Class options}\label{cls-opts}
  ...
  \section{Main matter}\label{sec:main-matter}
  ...
  \subsection{Boldface, italics}\label{subsec:bf-it}
  ...
  \subsection{Paragraphs}\label{subsec:para}
  ...
  \subsection{Non-breaking spaces}\label{subsec:non-breaking-space}\index{non-breaking space}
  Use \lstinline!~! for a non-breaking space: \lstinline!\$40~million!. 

  Use \Code{\\nobreakdash-} for a non-breaking hyphen: \lstinline!2013\nobreakdash-14!.

  \subsection{Footnotes}\label{subsec:footnote}\index{footnote}
  Use \lstinline!\\footnote{@@<text>@@}! to insert \lstinline!@@<text>@@! into a footnote.




  
  \subsection{Numbered / bulleted lists}\label{subsec:lists}\index{itemize}\index{enumerate}\index{lists}\index{bullets}
  ...
  \subsection{Quotes and dicta}\label{quotes-dicta}
  To insert an extended quote, use the \lstinline!quote! environment.

  \begin{lstlisting}
  \begin{quote}
    One of the reasons that clinicians struggle to form
    partnerships with patients and consumers is that 
    there is inadequate information for proper informed
    consent. The numbers that are collected don't filter
    through to clinicians dealing with the patient, and
    certainly not to the patient themselves. I encourage
    people to ask doctors three questions: `What are my
    options?', `What are the treatment outcomes -- both
    benefits and risks?', and `How likely are those
    outcomes to happen to me?'.  Doctors though say `We
    just don't have that information'.
  \end{quote}
  \end{lstlisting}

  \subsection{URLs}\label{subsec:URLs}
  In general, prefer URLs in the bibliography to the main text. 
  However, URLs can be inserted using \lstinline!\url{@@<url>@@}!. \index{URLs}

  Note that you should \emph{not} escape special characters inside \lstinline!\url!:

\begin{lstlisting}
% Bad (not a real URL):
\url{https://en.wikipedia.org/wiki/Grattan\_Institute}

% Correct
\url{https://en.wikipedia.org/wiki/Grattan_Institute}
\end{lstlisting}
  There is one exception: if the URL contains \%, you need to define it in the preamble using \lstinline!\urldef!.

  \index{URLs!if contains \%}
\begin{lstlisting}
\documentclass{grattan}
%...
\urldef\myURL{https://example.com/%20a}

\begin{document}
The URL will be printed: \myURL.
\end{document}
\end{lstlisting}
  ...
  \chapter{Floats: figures, tables and boxes}\label{chap:floats}
  \section{Figures}\label{sec:Figures}\index{figures}\index{charts}
...
\begin{table}[h]
\caption{Alignment parameters and their meanings}\label{tbl:complex-tbls}
...
\subsection{From Excel}\label{Excel2LaTeX}
...
\end{enumerate}
  ...
  \lstinline!\begin{tabular}{m{2cm}b{2cm}p{2cm}}! & starts a table with 3 columns vertical aligned to the middle, bottom, and top. \\
  \bottomrule
  \end{tabular}%
  %}%
  \end{adjustbox}
  ...
  \begin{enumerate}
    ...
    \item Use \verb=p= cells for the body, \verb=b= cells for the headings.
    \item Don't push tables too far.
    \begin{lstlisting}
% Wrong:
\begin{tabularx}{Xl}
...
\begin{tabularx}{\linewidth}{>{\bfseries}p{0.1\linewidth}lrRX>{\centering\arraybackslash}p{0.1\linewidth}}
  ...
  \subsection{\texttt{verysmallbox}}\label{subsec:verysmallbox}\index{verysmallbox}\index{boxes!verysmallbox}
  ...
  \subsection{\texttt{centredsmallbox}}\index{centredsmallbox}\index{boxes!centredsmallbox}
  The centred small box is used for a box the same size as \lstinline!smallbox! but centred on one page, rather than being constrained to the left or right half of the page.
  Use this if a \lstinline!smallbox! would otherwise be on a page on its own.
  
  \subsection{\texttt{addsmallbox}}\index{addsmallbox}\index{boxes!addsmallbox}
  Use the \lstinline!addsmallbox! environment for a \lstinline!smallbox! without box numbers.
  ...
  \subsubsection*{Figures in boxes must use [H]}\label{figs-in-boxes}
  ...
  \begin{enumerate}
    ...
    \item \lstinline!\Chapref! (and variants) same as \lstinline!\Cref! but mandatory for references to chapters \index{Chapref}\label{enum:Chapref}

    \begin{tabular}{ll}
    \toprule
    \textbf{Command} & \textbf{Prints as} \\
    \midrule
    \lstinline!\Chapref{chap:intro}! & Chapter 1 \\
    \lstinline!\Chapsref{chap:intro}! & Chapters 1 \\
    \lstinline!\topref{chap:chap3}! & 3 \\
    \lstinline!\Chaprefrange{chap:intro}{chap:chap3}! & Chapters 1 to 3 \\
    \lstinline!\Chaprefand{chap:intro}{chap:chap3}! & Chapters 1 and 3 \\
    \bottomrule
    \end{tabular}
  ...
  In \LaTeX, The database is a text file, customarily having file extension \lstinline!.bib!, containing entries for each work to be cited:
...
\subsection{Entering authors}\label{subsec:entering-bib-authors}
\begin{enumerate}
...
Enter the date if possible; the year if not. If you only have the month and year, you can enter the month as an integer.
  ...
  \subsection{\LaTeX{} commands to insert a citation}\label{subsec:insert-cite}
  ...
  \subsection{ShareLaTeX}\label{cite-in-sharelatex}
  ...
  Troubleshooting referencing errors are more difficult than pure \LaTeX{} errors, because the process involves a different program and this process occurs behind the scenes in Share\LaTeX{}.
  
  If you encounter error messages that include words like `malformed', most likely these problems can be fixed by rebuilding the cache.%
    \footnote{Such errors can occur if you interrupt a compile.} 
  In Share\LaTeX{}, click the button next to \textbf{Compile} with the orange number (or red number if there are errors), scroll to the bottom, hit the trash button, read the prompt and clear the cache.
  If you're compiling locally, you can achieve the same result by deleting all the auxiliary files in your project directory (\emph{viz.}~all the files with the same name as your \texttt{Report.tex} file, 
  but with extensions like \texttt{.aux}, \texttt{.blg}).

  If you get an error about ``too many commas'', you have probably entered an author or editor field with commas separating the authors, instead of \texttt{ and }. 
  That is, you've written 
  \begin{lstlisting}[language=BibTeX]
  % Wrong
  author = {John Daley, Danielle Wood, and Hugh Parsonage},
  \end{lstlisting}
  instead of 
  \begin{lstlisting}[language=BibTeX]
...
The package \texttt{soul} provides the commend \lstinline!\hl! which highlights text.
  ...
  \section{Avoiding a linebreak}
  To avoid a linebreak, you should first revisit the paragraph or paragraphs prior to see if any of the text can be modified. 
  (Remember that removing words may on rare occasions \emph{increase} the number of lines the paragraph occupies.)
  If you cannot reword the paragraph, you can use non-breaking spaces \eg~\lstinline!no~break! or use \lstinline!\mbox{no break}! which creates an unbreakable box.

  \subsection{In captions}\label{captionsetup}
  If a caption is slightly too long to fit on one line (say by one word), it is acceptable (indeed preferable) to modify the margins to allow the word to overflow into the margins.
  Use \lstinline!\captionsetup{oneside, margin={0em,@@<x>@@em}}!\index{captionsetup} for a suitable choice of \lstinline!@@<x>@@!, which equals the extra width in units of em-dashes by which the caption will expand.
...
\part{Style requirement: {\bfseries\texttt{grattanReporter}}}\label{part:grattanReporter}
...
\subsection{System requirements for release validation}\label{subsec:system-reqs-release-validation}
\begin{enumerate}
\item R and the package \texttt{grattanReporter}, installable via my drat repository:
<<install-grattanReporter>>=
if ("grattanReporter" %in% loadedNamespaces()) {
  stop("Restart R: packages currently loaded will need to be updated.")
}

myLibrary <- .libPaths()[1]
# If the library is on an UNC server \\uom.. it is not writeable:
if (identical(substr(myLibrary, 0, 2), c("\\", "\\"))) {
  myLibrary <- .libPaths()[2]
}
if (!is.na(myLibrary) && 
    #
    # i.e. \\uom...
    identical(substr(myLibrary, 0, 2), c("\\", "\\"))) {
  myLibrary <- tempdir()  # guaranteed to be writeable,
                          # if it's possible to install anything
}
install.packages("TeXCheckR", lib = myLibrary)
...
\item Ghostscript (\url{https://ghostscript.com/}) and the corresponding system environment set. For example in Windows, following successful installation of Ghostscript:\label{phantom:Sys-setenv-ghostscript}
<<set-RGSCMD>>=
...
All material published or otherwise created by Grattan Institute is licensed under a Creative Commons Attribution-NonCommercial-ShareAlike 3.0 Unported License\par
  ...
  By default, non-staff members are listed last.\fromversion{0.24.0}
  If you require the author to be placed ahead of staff members, use the directive 
  \begin{lstlisting}
add_author_to_recommended_citation_at: @@<author>@@ @@<order>@@
  \end{lstlisting}
  where \lstinline!@@<order>@@! is an integer specifying the position in the citation for the author.
...
\subsection{Quote marks}\label{subsec:quotation-marks}\index{quotation marks}\index{inverted commas}
\begin{enumerate}
...
(and any files inserted via \lstinline!\input! or \lstinline!\include! within that file). 
...
\item \textbf{\texttt{ignore\_spelling\_in\_file:}} You may use the \lstinline!% ignore_spelling_in_file: ! directive\fromversion{0.28.0}
followed by the path of a \texttt{.tex} file to ignore spelling of that entire file.
This should be used sparingly -- excessive use may throw an error in future.
 For example, ignoring all spelling in Appendix~A but checking spelling in Chapters 1 and 2:
\begin{lstlisting}
\documentclass{grattan}

\newcommand*{\hl}[1]{\textcolor{red}{#1}}
% ignore_spelling_in_file: chapters/appendixA.tex
% add_to_dictionary: skillset
...
\end{lstlisting}
the words \emph{and}, \emph{of}, \emph{the}, \emph{to}, and \emph{in}, are excluded when backtracking from the abbreviation. The following will still pass:
\begin{lstlisting}
...
If \texttt{grattanReporter} throws a spurious spelling error on a word that is likely to occur in other reports, please file this is as an issue at 

{\raggedleft\url{https://github.com/hughparsonage/grattanReporter/issues}} 
...
\begin{enumerate}
  ...
  If a figure or table does not need to be referenced,\fromversion{0.19.0} you may exclude it from the check using the \lstinline!% may_be_left_unreferenced:! directive.
  \begin{lstlisting}
  % OK:
...
@Misc{GST-Act-1999,
  ...
  \end{lstlisting}
    ...
    \begin{enumerate}
      \item Specifying a publisher for a \lstinline!@TechReport! or \lstinline!@Article!
      \item Failing to specify the institution field for a \lstinline!@TechReport!.
      \item Specifying ISBNs, ISSNs, etc when these are not appropriate.
    \end{enumerate}
\end{enumerate}
...
\lstinline[language=R]!checkGrattanReport(compile = TRUE)! will recompile the document as often as is required to stabilize cross-references.
In some cases, it is not possible with the text provided to do so. 
For example, consider a cross reference using \lstinline!\Vref! to the document's second figure on page 8. 
If \lstinline!\Vref! ends up in a sentence on the file line of page 9, \LaTeX{} has two options:
\begin{enumerate}
  \item Use the phrase ``Figure 2 on the previous page'' over page 9 and page 10; or
  \item Use the phrase ``Figure 2 on page 8'' on page 9.
\end{enumerate}
neither of which are appropriate.

In such an instance, you will receive an error message:\fromversion{0.20.0}\prereleaseonly
\begin{quote}
Unstable Vref. (LaTeX Warning: Label(s) may have changed. Rerun to get cross-references right.)
\end{quote}
above which there should be advice about which page boundaries have a straddling \lstinline!\Vref!. 
You should change these to \lstinline!\Cref! if appropriate or reword the paragraph if \lstinline!\Vref! is necessary.

\subsection{Ensuring \texttt{smallbox}es of a certain size do not intrude on the chapter title}\index{\texttt{smallbox!collision with chapter titles}}
If a box has around 22~lines of text, has a title/caption that extends over more than one line, occurs on the first page of a sufficiently large chapter,
you must use a \hyperref[subsec:verysmallbox]{\lstinline!verysmallbox!} (perhaps with the optional parameter \lstinline![p]!)
if the vertical position of the caption exceeds 166,160,000 nanometres from the bottom of the page 
(\ie~if the caption is set higher than the bottom of the space allocated for the chapter title).\fromversion{0.21.0}


\subsection{Checking placement of \texttt{\textbackslash CenturyFootnote}}\label{subsec:check-Century} \index{CenturyFootnote@\texttt{CenturyFootnote}}\index{footnotes!100th}
...
To include the front page proper, use \lstinline!\documentclass[FrontPage]{grattan}!.

To ensure the front page is of a sufficient resolution yet reasonable size, users of Windows and Linux can use Ghostscript.
(See \Vref{subsec:system-reqs-release-validation} for installation instructions.)

With \texttt{grattanReporter},\fromversion{0.25.0} you can run (from the project's working directory):
...
\appendix
\part{Error messages}\label{part:error-messages}
...
\section[\texttt{\textbackslash pdfendlink} ended up in different nesting level than \texttt{\textbackslash pdfstartlink}]{\mbox{\texttt{\textbackslash pdfendlink} ended up in different nesting level than \texttt{\textbackslash pdfstartlink}}}\index{pdfendlink}\index{pdfstartlink}
This error occurs when a hyperlink straddles a page boundary. 
When you get this error, you need to track down the page that this occurs on. 
Obviously if you've compiled recently, this will occur near the place you last edited, but may not be. 
Look for cross-references, citations, and URLs.

Reword the text.
Typically only a very minor adjustment to the wording is required.
However, if that proves too difficult, or if the error frequently reappears, you can set
\begin{lstlisting}
\hypersetup{draft}
\end{lstlisting}
to disable hyperlinks entirely. 
Note that this is only a temporary solution: before release you should unset this option; only rewording is appropriate.
...
The main thing is to ensure the affiliate columns are balanced.

\addchap{\texttt{grattan.cls}}



<<grattan-cls, results='asis', echo=FALSE, eval=TRUE>>=
cat("\\begin{lstlisting}[style=cls]\n")
cat(readLines("../grattan.cls"), sep = "\n")
cat("\\end{lstlisting}\n")
@
